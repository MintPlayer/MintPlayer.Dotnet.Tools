...
| D2 | `Pagination/MintPlayer.Pagination` | `TotalPages` divides by zero when `PerPage == 0`. A default request yields an empty `GetEffectiveSortColumns()`, so `OrderBySortColumns` throws — meaning `Paginate` on an unsorted request **always** throws. |
