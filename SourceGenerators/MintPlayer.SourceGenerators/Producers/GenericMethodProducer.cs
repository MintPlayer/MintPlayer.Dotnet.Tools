...
namespace MintPlayer.SourceGenerators.Producers
{
    public class GenericMethodProducer : Producer
    {
        ...
        protected override void ProduceSource(IndentedTextWriter writer, CancellationToken cancellationToken)
        {
            var methods = Methods
                .Where(m =>
                    m.Method is not null &&
                    m.Method.ClassModifiers.Any(Microsoft.CodeAnalysis.CSharp.SyntaxKind.PartialKeyword) &&
                    m.Method.MethodModifiers.Any(Microsoft.CodeAnalysis.CSharp.SyntaxKind.PrivateKeyword))
                .ToList();
            if (methods.Any())
            {
